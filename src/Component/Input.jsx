...
const Input = () => {
    // to check if a link has alredy been shortened
    const invalidlinks=(url)=>{
        // an array holding  major shoeting domains
        const shortenerDomain=[
        'tinyurl.com',
        'bit.ly',
        'short.link',
        't.co',
        ...
        try {
                        // getting the new url from the parameter
            const urlString = new URL(url)
            // trynna see if our new newUrl doesnt contain any of the domainName/ip above (in the array)
            if(shortenerDomain.some(domain => urlString.hostname.includes(domain))){
                console.log('user input an already shortened link')
                return true
            }else{
...
//    input(link) validation
   const handleSubmit=(e)=>{
        ...
        }else if(invalidlinks(input) === true){
            console.log('the input was an already shortened link');
            return setError('invaild link')
...
//  clear the local storage
    const clearLinkHistory=()=>{
        localStorage.clear('linkHistory')
        setLinkHistory([])
        setError('')
    }
  ...
  return (
    <div className=' bg-[hsl(0,0%,75%)] mt-25'>
        <div className='container mx-auto w-11/12 space-y-10 pb-10 relative '>
            ...
            {/* the link form */}
                <form action="post" onSubmit={handleSubmit} className='input flex md:flex-row flex-col gap-8 md:gap-3 relative'>
                    <input type="text" placeholder='Shorten a link here..' className={` ${error ? 'border-red-500' : 'border-green-200'} ${error ? 'placeholder-red-500' : 'placeholder-gray-400'} border-2 py-3 pl-3 text-gray-800 text-[12px] md:text-[18px] bg-white rounded-md md:w-[75%] text-[16px] outline-none focus:text-black`} value={input} onChange={(e)=>setInput(e.target.value)} />
                    <p className='absolute font-semibold text-red-500 top-[50px] md:top-[55px] left-0 italic md:text-[16px] text-[14px] '>{error}</p>
                    <button className={`bg-[hsl(180,66%,49%)] text-[18px] py-2 md:py-3 text-white font-semibold rounded-md md:w-[25%] hover:bg-[hsla(180,66%,67%,1.00)] cursor-pointer hover:text-[18.5px] `} onClick={handleSubmit}>{loading?'processing...':'Shorten it!'}</button>
                </form>
           </div>
        ...
        <div className='relative top-[-100px] md:top-[-70px]  left-[50%] -translate-x-1/2'>
            {/*mapped through fetched shorten links from linkHistory */}
            {
                linkHistory && linkHistory.map((link)=>(
                    <div key={link.id} className='py-1 flex md:flex-row flex-col bg-white mt-10 md:mt-5 md:justify-between md:text-balance md:items-center rounded-md '>
                    <p className='px-4 py-2 w-full md:w-[70%] overflow-hidden md:text-start text-[16px]'>{link.original}</p>
                    {/* the thin horizontal line on mobile */}
                    <div className='border border-gray-300 w-full md:hidden'></div>
                    <div className='px-4 flex flex-col py-2 md:flex-row items-center'>
                        <p className='md:px-2 w-full py-2 text-[hsl(180,66%,49%)] font-semibold text-[16px]'>{link.shortened}</p>
            ...
            {/* button to clear local storage */}
             <div className='flex justify-center w-full my-7'>
               { linkHistory.length > 0 && <button onClick={clearLinkHistory} className='  bg-red-400 px-5 py-2 rounded-md text-white font-semibold'>REMOVE ALL</button>}
            </div> 
        </div>
                ...
                <div className='relative flex container mx-auto'>
                    ...
                    {/* box1,2,3 */}
                            <div className='flex flex-col md:grid md:grid-cols-3 gap-14 md:gap-5 pt-10 z-3'>
                                {/* box 1 */}
                                <div className='container mx-auto w-11/12 flex flex-col items-center gap-3 bg-white text-center px-5 pt-12 py-10 rounded-sm text-balance md:pl-4 md:px-2 md:text-start relative'>
                                    ...
                                    <p className='text-gray-500 text-[15px] md:text-[18px]  text-balance md:text-start w-full'>Boost your brand recognition with each click. Generic links don’t mean a thing. Branded links help instil confidence in your content.
                                    </p>
                                </div>


                                {/* box 2 */}
                                <div className='container mx-auto w-11/12 flex flex-col items-center gap-3 bg-white text-center px-5 pt-12 py-10 rounded-sm text-balance md:pl-4 md:px-2 md:text-start relative md:top-8'>
                                    ...
                                    <p className='text-gray-500  text-[15px] md:text-[18px]  text-balance md:text-start w-full'>Gain insights into who is clicking your links. Knowing when and where people engage with your content helps inform better decisions.
                                    </p>
                                </div>

                                {/* box 3 */}

                                <div className='container mx-auto w-11/12 flex flex-col items-center gap-3 bg-white text-center px-5 pt-12 py-10 rounded-sm text-balance md:pl-4 md:px-2 md:text-start relative md:top-15'>
                                    ...
                                    <p className='text-gray-500 text-[15px] md:text-[18px] text-balance md:text-start w-full'>Improve brand awareness and content discoverability through customizable links, supercharging audience engagement.
                                    </p>
                                </div>
                            </div>
                </div>
        ...
        <div className='boost-section bg-[hsl(257,27%,26%)] text-white text-center font-semibold py-18 space-y-2'>
                ...
                <button className='bg-[hsl(180,66%,49%)] py-2 px-10 rounded-full text-[20px] hover:bg-[hsla(180,66%,67%,1.00)] cursor-not-allowed hover:text-[20.5px]'>
                Get Started
                </button>
                </div>
        </div>
